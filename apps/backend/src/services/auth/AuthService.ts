...
import { auditService, AuditService } from '../audit/AuditService.js';
import { emailService, EmailService } from '../email/EmailService.js';

import { tokenService, TokenService } from './TokenService.js';
...
export class AuthService {
  ...
  async verifyEmail(
    token: string,
    meta: { ipAddress?: string | undefined },
  ): Promise<{ message: string }> {
  ...
  async getMe(userId: string): Promise<PublicUser> {
    const user = await db.identity.user.findFirst({
...
export function createAuthService(): AuthService {
  return new AuthService(new TokenService(), new EmailService(), new AuditService());
}
